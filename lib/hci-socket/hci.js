...
var OCF_LE_CREATE_CONN_CANCEL = 0x000e;
...
var LE_CREATE_CONN_CANCEL_CMD = OCF_LE_CREATE_CONN_CANCEL | OGF_LE_CTL << 10;
...
var BLE_GAP_CP_MIN_CONN_INTVL_MIN = 0x0006;
var BLE_GAP_CP_MIN_CONN_INTVL_MAX = 0x0C80;
var BLE_GAP_CP_MAX_CONN_INTVL_MIN = 0x0006;
var BLE_GAP_CP_MAX_CONN_INTVL_MAX = 0x0C80;
var BLE_GAP_CP_SLAVE_LATENCY_MAX = 0x01F3;
var BLE_GAP_CP_CONN_SUP_TIMEOUT_MIN = 0x000A;
var BLE_GAP_CP_CONN_SUP_TIMEOUT_MAX = 0x0C80;
...
Hci.prototype.setScanParameters = function() {
  ...
  cmd.writeUInt8(process.env.NOBLE_BLE_SCAN_TYPE ? parseInt(process.env.NOBLE_BLE_SCAN_TYPE, 10) : 1, 4)
  // --MBIENT
...
Hci.prototype.createLeConn = function(address, addressType, options) {
  ...
  var minInterval = options.hasOwnProperty('minInterval') ? options.minInterval : 0x0006;
  minInterval = Math.max(minInterval, BLE_GAP_CP_MIN_CONN_INTVL_MIN);
  minInterval = Math.min(minInterval, BLE_GAP_CP_MIN_CONN_INTVL_MAX);

  var maxInterval = options.hasOwnProperty('maxInterval') ? options.maxInterval : 0x000c;
  maxInterval = Math.max(maxInterval, BLE_GAP_CP_MAX_CONN_INTVL_MIN);
  maxInterval = Math.min(maxInterval, BLE_GAP_CP_MAX_CONN_INTVL_MAX);
  maxInterval = Math.max(maxInterval, minInterval);

  var latency = options.hasOwnProperty('latency') ? options.latency : 0x0000;
  latency = Math.min(latency, BLE_GAP_CP_SLAVE_LATENCY_MAX);

  var supervisionTimeout = options.hasOwnProperty('supervisionTimeout') ? options.supervisionTimeout : 0x00c8;
  supervisionTimeout = Math.max(supervisionTimeout, BLE_GAP_CP_CONN_SUP_TIMEOUT_MIN);
  supervisionTimeout = Math.min(supervisionTimeout, BLE_GAP_CP_CONN_SUP_TIMEOUT_MAX);
...
Hci.prototype.createLeConnCancel = function() {
  var cmd = new Buffer(4);

  // header
  cmd.writeUInt8(HCI_COMMAND_PKT, 0);
  cmd.writeUInt16LE(LE_CREATE_CONN_CANCEL_CMD, 1);
