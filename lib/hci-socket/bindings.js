...
NobleBindings.prototype.connect = function(peripheralUuid, options) {
  ...
  if (!this._pendingConnectionUuid) {
    ...
    this._hci.createLeConn(address, addressType, options);
  } else {
...
NobleBindings.prototype.spoofPeripheral = function(address, addressType, connectable, advertisement, rssi) {
  var uuid = address.split(':').join('');
  this._addresses[uuid] = address;
  this._addresseTypes[uuid] = addressType;
  this._connectable[uuid] = connectable;
...
NobleBindings.prototype.onLeConnComplete = function(status, handle, role, addressType, address, interval, latency, supervisionTimeout, masterClockAccuracy) {
  ...
  if (this._connectionQueue.length > 0) {
    var config = this._connectionQueue.shift();

    address = this._addresses[config.peripheralUuid];
    addressType = this._addresseTypes[config.peripheralUuid];

    this._pendingConnectionUuid = config.peripheralUuid;

    this._hci.createLeConn(address, addressType, config.options);
  } else {
    this._pendingConnectionUuid = null;
  }
};
